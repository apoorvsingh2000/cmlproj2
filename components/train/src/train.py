...
def load_feature(input_x_path):
    with gfile.GFile(input_x_path, 'rb') as input_x_file:
        return pickle.loads(input_x_file.read())

def load_label(input_y_path):
    with gfile.GFile(input_y_path, 'rb') as input_y_file:
...
with open(MODEL_FILE, 'rb') as input_f:
    with gfile.GFile(args.output_model_path + '/' + MODEL_FILE, 'wb') as output_f:
...
with gfile.GFile('/mlpipeline-metrics.json', 'w') as f:
    json.dump(metrics, f)

# Write out TensorBoard metadata
metadata = {
    'outputs': [{
        'type': 'tensorboard',
...
with gfile.GFile('/mlpipeline-ui-metadata.json', 'w') as f:
    json.dump(metadata, f)

# Write output model path
